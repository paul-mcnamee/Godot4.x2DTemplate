...
var max_health_overflow: float:
	get:
		return max_health + (max_health * health_overflow_percentage / 100)

var can_overflow: bool = max_health < max_health_overflow
...
func health(amount: float, type: TYPES = TYPES.HEALTH):
	amount = absi(amount)
...
func check_is_dead() -> bool:
	...
	if is_dead:
		GodotLogger.debug("ded parent = ", self.get_parent())
		died.emit()
...
func get_health_percent() -> Dictionary:
	...
	return {
		...
		"overflow_health": maxf(0, current_health - max_health)
	}
...
func enable_health_regen(amount_per_second: float = health_regen_per_second):
...
func on_health_changed(amount: float, type: TYPES):
