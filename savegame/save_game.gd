...
static func save_game(tree:SceneTree):
	...
	var save_state : FileAccess

	if OS.is_debug_build():
		save_state = FileAccess.open("user://" + SAVE_GAME_TEMPLATE, FileAccess.WRITE)
	else:
		save_state = FileAccess.open_encrypted_with_pass("user://" + SAVE_GAME_TEMPLATE, FileAccess.WRITE, ENCRYPTION_KEY)
	...
	for node in save_nodes:
		...
		save_state.store_line(JSON.stringify(save_data))
...
static func load_game(tree:SceneTree) -> void:
	...
	if OS.is_debug_build():
		save_state = FileAccess.open("user://" + SAVE_GAME_TEMPLATE, FileAccess.READ)
	else:
		save_state = FileAccess.open_encrypted_with_pass("user://" + SAVE_GAME_TEMPLATE, FileAccess.READ, ENCRYPTION_KEY)

	while save_state.get_position() < save_state.get_length():
		# Get the saved dictionary from the next line in the save file
		var test_json_conv = JSON.new()
		test_json_conv.parse(save_state.get_line())
